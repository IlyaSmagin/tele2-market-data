...
async function getSmsData(data_length: number, treshhold = 100000, volume?: number) {
	...
	if (volume !== undefined) {
		const { data, error } = await supabase
			.from("MarketData")
			.select("created_at, sms!inner(count)")
			.eq("sms.volume", volume)
			.gt("sms.count", `${treshhold}`)
		...
		const graphData = (data as any[])
			.map((row) => ({
				date: row.created_at,
				numberOfLots: row.sms?.[0]?.count ?? 0,
			}))
			.reverse();
		return graphData;
	}
